$axure.internal(function($ax) {
    ...
    var _computeAllOverrides = $ax.style.computeAllOverrides = function(id, parentId, state, currentViewId) {
        ...
        while(state) {
            ...
            $.extend(computedStyle, _computeStateStyleForViewChain(diagramObject, currState, viewIdChain));
            state = _progessState(state);
        }
    ...
    var _setTextAlignment = function(textId, alignProps) {
        ...
        if(oldTop != newTop) $jobj(textId).css('top', newTop + 'px');
    };
    ...
    $ax.style.clearAdaptiveStyles = function() {
        for(var shapeId in _adaptiveStyledWidgets) {
            ...
            _applyImageAndTextJson(shapeId, $ax.style.generateState(shapeId));
        }
